...
const authOf = (req) => req.auth ?? sysCtx();
...
function send(res, fn) {
    ...
    Promise.resolve().then(fn).then((data) => res.json(data), (e) => {
...
app.post("/api/entries/expense", (req, res) => send(res, () => logExpense(LogEntrySchema.parse(req.body), authOf(req))));
app.post("/api/entries/income", (req, res) => send(res, () => logIncome(LogEntrySchema.parse(req.body), authOf(req))));
app.get("/api/summary", (req, res) => send(res, () => getSummary(GetSummarySchema.parse({
    period: req.query.period,
    category: req.query.category,
}), authOf(req))));
app.get("/api/breakdown", (req, res) => send(res, () => getCategoryBreakdown(CategoryBreakdownSchema.parse({ period: req.query.period }), authOf(req))));
app.get("/api/entries", (req, res) => send(res, () => listEntries(ListEntriesSchema.parse({
    date_from: req.query.date_from,
    date_to: req.query.date_to,
    category: req.query.category,
    payment_method: req.query.payment_method,
    is_income: req.query.is_income === undefined
        ? undefined
        : req.query.is_income === "true",
    limit: req.query.limit === undefined ? undefined : Number(req.query.limit),
}), authOf(req))));
app.patch("/api/entries/:id", (req, res) => send(res, () => editEntry(EditEntrySchema.parse({ ...req.body, id: Number(req.params.id) }), authOf(req))));
app.delete("/api/entries/:id", (req, res) => send(res, () => deleteEntry(DeleteEntrySchema.parse({ id: Number(req.params.id) }), authOf(req))));
