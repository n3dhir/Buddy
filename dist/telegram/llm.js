...
const numField = z.preprocess((v) => (typeof v === "string" && v.trim() !== "" && Number.isFinite(Number(v)) ? Number(v) : v), z.number().nullable().optional());
const cleanStr = z.preprocess((v) => (typeof v === "string" && v.trim() === "" ? null : v), z.string().nullable().optional());
...
export async function parseFreeText(text, ctx) {
    ...
    if (!res.ok)
        throw new LLMUnavailable(`Ollama HTTP ${res.status} at ${host}`);
    let data;
    try {
        data = await res.json();
    }
...
export function missingFields(intent) {
    const missing = [];
    switch (intent.action) {
        ...
        case "get_category_breakdown":
            if (!intent.period)
                missing.push("period (week/month/year)");
            break;
        case "delete_entry":
            if (!intent.entry_id)
                missing.push("entry id (#N)");
            break;
        default:
            break;
    }
...
export function formatProposal(intent) {
