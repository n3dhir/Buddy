...
import { isValidDate, normCat, normalizeRow, nowTunisDateTime, resolvePeriod, round2, todayISO, } from "./utils.js";
...
export async function getSummary(input, ctx = sysCtx()) {
    ...
    if (input.category)
        q.andWhere({ category: normCat(input.category) });
...
export async function getCategoryBreakdown(input, ctx = sysCtx()) {
    ...
    const { from, to } = resolvePeriod(input.period);
    const db = getDb();
    const rows = await scope(db("transactions").whereBetween("date", [from, to]).andWhere({ is_income: false }), ctx)
        .select("category")
        .sum({ total: "amount" })
        .count({ count: "id" })
        .groupBy("category")
...
export async function listEntries(input, ctx = sysCtx()) {
    ...
    if (input.date_from)
        q.andWhere("date", ">=", input.date_from);
    if (input.date_to)
        q.andWhere("date", "<=", input.date_to);
    if (input.category)
        q.andWhere({ category: normCat(input.category) });
    if (input.payment_method)
        q.andWhere({ payment_method: input.payment_method });
    if (input.is_income !== undefined)
        q.andWhere({ is_income: input.is_income });
...
export async function editEntry(input, ctx = sysCtx()) {
    ...
    if (input.category !== undefined)
        patch.category = normCat(input.category);
    ...
    if (input.is_income !== undefined)
        patch.is_income = input.is_income;
